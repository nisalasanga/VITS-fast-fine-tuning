  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(tqdm(train_loader)):
    ...
    if rank==0:
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        if os.path.exists("/content/drive/MyDrive/Trained_model/Model_checkpoints/"):
            utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
                                  os.path.join("/content/drive/MyDrive/Trained_model/Model_checkpoints/", "G_latest.pth"))

            utils.save_checkpoint(net_d, None, hps.train.learning_rate, epoch,
                                  os.path.join("/content/drive/MyDrive/Trained_model/Model_checkpoints/", "D_latest.pth"))
        if hps.preserved > 0:
          utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
          ...
          if os.path.exists("/content/drive/MyDrive/Trained_model/Model_checkpoints/"):
              utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
                                    os.path.join("/content/drive/MyDrive/Trained_model/Model_checkpoints/", "G_{}.pth".format(global_step)))
              utils.save_checkpoint(net_d, None, hps.train.learning_rate, epoch,
                                    os.path.join("/content/drive/MyDrive/Trained_model/Model_checkpoints/", "D_{}.pth".format(global_step)))
              old_g = utils.oldest_checkpoint_path("/content/drive/MyDrive/Trained_model/Model_checkpoints/", "G_[0-9]*.pth",
                                                   preserved=hps.preserved)  # Preserve 4 (default) historical checkpoints.
              old_d = utils.oldest_checkpoint_path("/content/drive/MyDrive/Trained_model/Model_checkpoints/", "D_[0-9]*.pth", preserved=hps.preserved)
              if os.path.exists(old_g):
                  print(f"remove {old_g}")
                  os.remove(old_g)
              if os.path.exists(old_d):
